...
H_left = -J*np.kron(Sz,Sz) - Jx*np.kron(Sx,Sx) - Jy*np.kron(Sy,Sy) - hx*np.kron(Sx,np.identity(2)) - 0.5*hx*np.kron(np.identity(2),Sx) 
H_right = -J*np.kron(Sz,Sz) - Jx*np.kron(Sx,Sx) - Jy*np.kron(Sy,Sy) - hx*np.kron(np.identity(2),Sx) - 0.5*hx*np.kron(Sx,np.identity(2))

Uf = np.reshape(lin.expm(-1j*H*delta) ,(2,2,2,2))
Uh = np.reshape(lin.expm(-1j*H*delta/2) ,(2,2,2,2))
#Left-boundary
Uh_left = np.reshape(lin.expm(-1j*H_left*delta/2) ,(2,2,2,2))
Uh_right = np.reshape(lin.expm(-1j*H_right*delta/2) ,(2,2,2,2))
#Right-boundary
Uf_right = np.reshape(lin.expm(-1j*H_right*delta) ,(2,2,2,2))
    ...
    for ii in range(0,N-1):
        ...
        if np.mod(step,3) == 1: #Even Gates
            ...
            if ii%2 == 1:
                ...
                if ii == N-2: #Right end of chain
                    U = copy.deepcopy(Uf_right)
            ...
            if ii%2 == 0:
                ...
                if ii == 0: #Left end of chain
                    U = copy.deepcopy(Uh_left)
         
                elif ii == N-2: #Right end of chain
                    U = copy.deepcopy(Uh_right)
